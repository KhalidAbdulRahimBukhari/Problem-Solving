...
void FillArrayWithSumOfRowsInMatrix(int matrix[3][3], int arr[], int rows, int cols)
{
	for (int i = 0; i < rows; i++)
	{
...
void PrintSumOfEachRowFromArray(int matrix[3][3],int arr[3], int rows, int cols)
{
	for (int i = 0; i < rows; i++)
	{
...
int main()
{
	...
	int matrix[3][3], rows = 3, cols = 3, arr[3];
	...
	FillArrayWithSumOfRowsInMatrix(matrix, arr, rows, cols);
	cout << "The following are the sum of each row in matrix : " << endl;
	PrintSumOfEachRowFromArray(matrix, arr, rows, cols);
